...
class PrivateKeyEthSigner : EthSigner {
    ...
    override fun <S : Structurable> signTypedData(domain: Eip712Domain, typedData: S): String {
        val message = Eip712Encoder.typedDataToSignedBytes(domain, typedData)
        return signMessage(message, false)
    }

    override fun <S : Structurable> verifyTypedData(domain: Eip712Domain, typedData: S, signature: String): Boolean {
        val message = Eip712Encoder.typedDataToSignedBytes(domain, typedData)
        return verifySignature(signature, message, false)
    }
    ...
    override fun signMessage(message: ByteArray, addPrefix: Boolean): String {
        ...
        val signature = Numeric.toHexString(output.toByteArray())
        return signature
    }
    ...
    override fun verifySignature(signature: String, message: ByteArray, prefixed: Boolean): Boolean {
        val messageHash = if (prefixed) EthSigner.getEthereumMessageHash(message) else message
    ...
    companion object {
        ...
        private fun ecrecover(signature: ByteArray, hash: ByteArray): String {
            val sig = ECDSASignature(
                Numeric.toBigInt(signature.copyOfRange(0, 32)),
                Numeric.toBigInt(signature.copyOfRange(32, 64))
            )
            val v = signature[64]
            val recId = if (v >= 3) v - 27 else v.toInt()
            val recoveredKey = Sign.recoverFromSignature(recId, sig, hash)
